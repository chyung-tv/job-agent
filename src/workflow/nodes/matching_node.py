...
class MatchingNode(BaseNode):
    ...
    def _validate_context(self, context: JobSearchWorkflowContext) -> bool:
        ...
        if not context.jobs and not context.job_search_id:
            context.add_error("Jobs list or job_search_id is required for matching step")
            return False
        return True
...
"""
    ...
    async def _match_job(self, user_profile: str, job: JobResult) -> Optional[JobScreeningOutput]:
    ...
    def _persist_data(self, context: JobSearchWorkflowContext, session) -> None:
        ...
        for screening_output in context.matched_results:
            ...
            if not job_id_to_use or job_id_to_use.strip() == "":
                if screening_output.application_link and screening_output.application_link.get("link"):
                    ...
                    self.logger.info(f"Generated job_id from application_link for: {screening_output.job_title}")
                else:
                    self.logger.warning(f"Matched job missing job_id and application_link, skipping: {screening_output.job_title}")
                    skipped_count += 1
                    continue
            ...
            if not job_posting_id:
                for posting in postings:
                    if (posting.title == screening_output.job_title and 
                        ...
                        self.logger.info(f"Found JobPosting by title+company match for: {screening_output.job_title}")
                        break
            
            if not job_posting_id:
                self.logger.warning(f"JobPosting not found for job_id: {job_id_to_use} (title: {screening_output.job_title})")
                skipped_count += 1
                continue
        ...
        self.logger.info(f"Saved {saved_count}/{len(context.matched_results)} matched jobs")
        if skipped_count > 0:
    ...
    async def run(self, context: JobSearchWorkflowContext) -> JobSearchWorkflowContext:
        ...
        for job in jobs_to_screen:
            result = await self._match_job(context.user_profile, job)
            if not result:
                continue

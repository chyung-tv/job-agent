...
class ResearchNode(BaseNode):
    ...
    def __init__(
        self, use_streaming: bool = False, max_retries: int = DEFAULT_MAX_RETRIES
    ):
    ...
    def _get_research_by_job_posting(
        self, session: Session, job_posting_id: str
    ...
    async def _research_company_for_job(
        ...
        existing_research = self._get_research_by_job_posting(
            session, str(job_posting.id)
        )
...
"""
            ...
            self.logger.info(
                f"Researching: {job_posting.company_name} - {job_posting.title}"
            )
            ...
            error_msg = str(e)
            matched_job.research_status = (
                "failed"
                if matched_job.research_attempts >= self.max_retries
                else "pending"
            )
            ...
            session.commit()
            self.logger.error(
                f"Research failed (attempt {matched_job.research_attempts}/{self.max_retries}): {error_msg}"
            )
            return None
    ...
    async def _execute(
        ...
        try:
            matched_jobs = (
                session.query(MatchedJob).filter_by(run_id=context.run_id).all()
            )
            ...
            for matched_job in matched_jobs:
                ...
                if (
                    matched_job.research_attempts >= self.max_retries
                    and matched_job.research_status == "failed"
                ):
                    failed += 1
                    continue
                ...
                if not job_posting:
                    self.logger.error(
                        f"Job posting {matched_job.job_posting_id} not found"
                    )
                    matched_job.research_status = "failed"
                    matched_job.research_error = (
                        f"Job posting {matched_job.job_posting_id} not found"
                    )
                    matched_job.research_attempts += 1
                    failed += 1
                ...
                # Perform research
                result = await self._research_company_for_job(
                    session, matched_job, job_posting
                )
                if result:
                    successful += 1
                    self.logger.info(
                        f"Research completed for {job_posting.title} at {job_posting.company_name}"
                    )
                else:
                    failed += 1

            self.logger.info(
                f"Research completed: {successful} successful, {failed} failed"
            )

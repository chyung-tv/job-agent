...
class NylasService:
    ...
    def _get_user_profile(
        self, session: Session, user_email: Optional[str] = None
    ) -> Optional[Dict]:
    ...
    def _prepare_email_data(
        self, completed_items: List[Dict], user_profile: Optional[Dict] = None
    ) -> Dict:
        ...
        for item in completed_items:
            ...
            if isinstance(application_link, dict):
                application_link = application_link.get("link")
            job_data = {
    ...
    def send_job_application_email(
        ...
        try:
            ...
            email_to = recipient_email or (
                user_profile.get("email") if user_profile else None
            )
            if not email_to:
                raise ValueError(
                    "No recipient email provided and no user profile found"
                )
            ...
            logger.info(
                f"Sending email to {email_to} with {len(completed_items)} job application(s)"
            )
